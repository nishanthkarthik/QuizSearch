...
module.exports = new function(){
	this._data = fs.readFileSync('data/fse.txt', {encoding: 'utf8'});
	...
	this.readBlocks = function(data, pattern, string){
		if(data != null){
			...
			while(index !== -1){
				...
				index = next === -1 ? -1 : next;
				// console.log(string.trim().substring(0, 300));
				// i++;
			}
			return found;
	...
	this.readParts = function(data){
		return this.readBlocks(data, /^\s{15}[0-9]{1,2}\s{1,4}([^\s]+\s){1,5}\s*$/gm, '');
	}


	this.readParagraphs = function(data, part, article){
		return this.readBlocks(data, /^\s{15}[0-9]{1,2}\.[0-9]+.*$/gm, 'Xn.');
	}
